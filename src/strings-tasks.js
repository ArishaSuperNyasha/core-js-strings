...
function countVowels(str) {
  ...
  for (let i = 0; i < str.length; i += 1) {
    if (dict.includes(str[i])) count += 1;
  }
  return count;
}
...
function isPalindrome(str) {
  const newS = str.toLowerCase().replace(/[,.!? ]/g, '');
  const len = newS.length;
  for (let i = 0; i < Math.round(len / 2 + 1); i += 1) {
...
function reverseWords(str) {
  ...
  for (let w = 0; w < words.length; w += 1) {
    let newS = '';
    for (let i = words[w].length - 1; i > -1; i -= 1) {
      newS = newS.concat(words[w][i]);
    }
    arr.push(newS);
  }
  return arr.join(' ');
...
function encodeToRot13(str) {
  ...
  for (let i = 0; i < str.length; i += 1) {
    const symb = str[i];
    if (arr1.includes(symb)) {
      newS += arr2[arr1.indexOf(symb)];
    } else {
      newS += symb;
    }
  }
...
function getCardId(value) {
  ...
  const d = value.match(/[♣♦♥♠]/u);
  const a = value.split(/[♣♦♥♠]/u).join('');
  return arr.indexOf(a) + s[d];
}
